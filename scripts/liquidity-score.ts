...
function parseArgs(argv: string[]) {
  ...
  for (const arg of argv) {
    if (arg === "--all") {
      ...
      options.includeSec = parseBoolean(arg.split("=")[1]);
    } else {
      throw new Error(
        `Unknown liquidity:score option "${arg}". Use --all, --theme=..., --ticker=..., --massive=off, --fmp=off, or --sec=off.`,
      );
    }
  }
...
async function main() {
  ...
  try {
    const result = await scoreThemeLiquidity(prisma, options);
      ...
      JSON.stringify(
        {
          ...
          sec_configured: result.secConfigured,
